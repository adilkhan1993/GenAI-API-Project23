...
CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8000"]
    ```
    **Обязательно сохраните файл** (`Ctrl + S`).
  
  
    Теперь возвращайтесь в черное окно **PowerShell**. Так как мы изменили "рецепт" в файле, нам нужно сказать Docker заново "испечь" образ приложения. Введите:
    ```bash
    docker build -t genai-api .
    ```
    *(Точка в конце обязательна — она говорит Docker искать файл в текущей папке)*.
  
  
    Когда сборка успешно завершится, запустите контейнер вашей обычной командой: